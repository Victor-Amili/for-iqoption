...
STARTING_BALANCE = 1000.0  # Virtual balance tracking for backtest profit computation
...
# ============================================================
# ALL 7 STRATEGY SIGNAL LOGIC (VECTORIZED FOR BACKTESTING)
# ============================================================

def evaluate_strategy_signals(candles):
    ...
    # Initialize signal columns for all 7 strategies
    for s_id in range(1, 8):
        df[f'sig_{s_id}'] = None

    # Evaluate candles starting from index 100 to ensure indicator stability
    for i in range(100, len(df)):
        row = df.iloc[i]
        prev = df.iloc[i-1]
        
        # Strategy 1: BB + RSI Scalp
        ...
        # Strategy 2: MACD + SuperTrend / Vortex confluence placeholder variant
        if row['close'] > row['ema100'] and row['histogram'] > 0 and row['histogram'] > prev['histogram']:
            df.loc[i, 'sig_2'] = 'call'
        elif row['close'] < row['ema100'] and row['histogram'] < 0 and row['histogram'] < prev['histogram']:
            df.loc[i, 'sig_2'] = 'put'
        ...
        # Strategy 5: Momentum Breakout (Donchian/High-Low Breakout)
        if row['close'] > df['high'].iloc[i-20:i].max():
            df.loc[i, 'sig_5'] = 'call'
        elif row['close'] < df['low'].iloc[i-20:i].min():
            df.loc[i, 'sig_5'] = 'put'

        # Strategy 6: Stochastic Oscillator Reversal (%K / %D)
        ...
        if row['close'] > sma50 and row['low'] <= sma50:
            df.loc[i, 'sig_7'] = 'call'
        elif row['close'] < sma50 and row['high'] >= sma50:
            df.loc[i, 'sig_7'] = 'put'
...
def run_backtest():
    ...
    send_telegram_message("⏳ <b>Backtest Started</b>\nSimulating all 7 strategies across OTC assets...")

    strategy_names = {
        1: "Strategy 1 (BB + RSI)",
        2: "Strategy 2 (MACD Trend)",
        3: "Strategy 3 (RSI Extreme)",
        4: "Strategy 4 (EMA Cross)",
        5: "Strategy 5 (Breakout)",
        6: "Strategy 6 (Stochastic)",
        7: "Strategy 7 (MA Pullback)"
    }

    # Initialize stats tracking dictionary for all 7 strategies
    results = {s_id: {
        "name": name,
        "trades": 0,
        "base_wins": 0,
    ...
    for pair in OTC_PAIRS:
        ...
        candles = API.get_candles(pair, 60, 600, time.time()) # Pulling last 600 candles
        if not candles or len(candles) < 150:
            continue
            
        df = evaluate_strategy_signals(candles)
        ...
        for i in range(100, len(df) - 1):
            for s_id in range(1, 8):
                ...
                if sig is not None:
                    # Check outcome on the next candle (i+1)
                    ...
                    if is_win:
                        st["base_wins"] += 1
                        profit = BASE_STAKE * payout_rate
                        st["balance"] += profit
                    else:
                        st["base_losses"] += 1
                        # Martingale Step simulation
                        st["balance"] -= BASE_STAKE
                        mg_stake = BASE_STAKE * MARTINGALE_MULTIPLIER
                        
                        # Check outcome on candle i+2 for martingale recovery
                        if i + 2 < len(df):
                            mg_entry = df.loc[i+1, 'close']
                            mg_exit = df.loc[i+2, 'close']
                            mg_win = (sig == 'call' and mg_exit > mg_entry) or (sig == 'put' and mg_exit < mg_entry)
                            
                            if mg_win:
                                st["mg_wins"] += 1
                                mg_profit = (mg_stake * payout_rate) - BASE_STAKE
                                st["balance"] += mg_profit
                            else:
                                st["mg_losses"] += 1
                                st["balance"] -= mg_stake

    # Compile Final Report Message
    report_lines = ["📊 <b>FIREFLY BACKTEST RESULTS (ALL 7 STRATEGIES)</b>\n"]
    
    for s_id, st in results.items():
        total_wins = st["base_wins"] + st["mg_wins"]
        total_resolved = st["trades"] if st["trades"] > 0 else 1
        win_rate = (total_wins / total_resolved) * 100
        total_profit = st["balance"] - STARTING_BALANCE

        report_lines.append(
            f"<b>{st['name']}</b>\n"
            f"• Total Trades Taken: {st['trades']}\n"
            f"• Base Wins: {st['base_wins']} | Base Losses: {st['base_losses']}\n"
            f"• Martingale Wins: {st['mg_wins']} | Martingale Losses: {st['mg_losses']}\n"
    ...
    logger.info("Backtest complete. Report dispatched to Telegram.")
